...
import { ArrowLeft, Search, Filter, UtensilsCrossed, Sparkles, Smartphone, Shirt, Stethoscope, Home, Film, Trophy } from "lucide-react";
import { useNavigate } from "react-router-dom";

const categories = [
  { id: "restaurants", name: "Restaurantes", icon: UtensilsCrossed, color: "text-red-700" },
  { id: "beauty", name: "Belleza", icon: Sparkles, color: "text-pink-700" },
  { id: "technology", name: "Tecnología", icon: Smartphone, color: "text-blue-700" },
  ...
  { id: "entertainment", name: "Entretenimiento", icon: Film, color: "text-indigo-700" },
...
export default function Categories() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const couponsData = await Coupon.filter({ is_active: true }, "-created_date", 50);
  ...
  const filteredCoupons = coupons.filter(c => {
    ...
    const matchesSearch = !searchTerm || 
      ...
      businesses[c.business_id]?.name.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      <div className="px-4 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          {categories.map((category) => {
            const categoryCount = coupons.filter(c => c.category === category.id).length;
            ...
            return (
              ...
              >
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-xs text-gray-500 mt-1">{categoryCount} cupones</p>
                </CardContent>
              </Card>
            );
          })}
            ...
            <div className="flex items-center space-x-3">
              <div className="text-2xl">
                 {(() => {
                    ...
                    return <Icon className={`w-7 h-7 ${selectedCategoryInfo.color}`} />;
                  })()}
              </div>
              <div>
                <h2 className="text-xl font-bold text-gray-900">{selectedCategoryInfo.name}</h2>
                <p className="text-sm text-gray-500">
                  {filteredCoupons.length} cupones {searchTerm ? 'encontrados' : 'disponibles'}
                </p>
              </div>
            </div>
            <Button 

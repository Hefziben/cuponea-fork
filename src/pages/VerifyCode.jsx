...
import { ArrowLeft, Crown, CheckCircle, AlertCircle, Search } from "lucide-react";
...
export default function VerifyCode() {
  ...
  const handleVerifyCode = async (e) => {
    ...
    try {
      ...
      const cuponeadores = await Cuponeador.filter({ cuponeador_code: cuponeadorCode.toUpperCase() });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-green-50 px-4 py-8">
      <div className="max-w-md mx-auto">
        ...
        {verificationResult && (
          <Card className={`mt-6 ${verificationResult.success ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
              ...
              <div className="text-center space-y-4">
                {verificationResult.success ? (
                  <>
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-green-800">¡Código Válido!</h3>
                      <p className="text-green-700 mt-1">{verificationResult.message}</p>
                    </div>
                    
                    {verificationResult.user && (
                      <div className="bg-white rounded-lg p-4 border border-green-200">
                        <h4 className="font-semibold text-gray-900 mb-2">Información del Cuponeador:</h4>
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between">
                            <span className="text-gray-600">Nombre:</span>
                            <span className="font-medium">{verificationResult.user.full_name}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Código:</span>
                            <span className="font-mono font-bold">{verificationResult.cuponeador.cuponeador_code}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Nivel:</span>
                            <Badge className={`${
                              verificationResult.cuponeador.level === 'oro' ? 'bg-yellow-100 text-yellow-800' :
                              verificationResult.cuponeador.level === 'plata' ? 'bg-gray-100 text-gray-800' :
                              verificationResult.cuponeador.level === 'diamante' ? 'bg-purple-100 text-purple-800' :
                  ...
                  <>
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-red-800">Código No Válido</h3>
                      <p className="text-red-700 mt-1">{verificationResult.message}</p>
                    </div>
                    <p className="text-sm text-red-600">
                      Verifica que hayas ingresado el código correctamente o contacta a tu cuponeador.
                    </p>
                  </>
                )}
              </div>
        ...
        <Card className="mt-6 bg-blue-50 border-blue-200">
          <CardContent className="p-4">
            <h3 className="font-semibold text-blue-800 mb-2">¿Para qué sirve este código?</h3>

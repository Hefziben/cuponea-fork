      ...
      <div className="bg-blue-50 border-2 border-blue-300 rounded-lg p-4 space-y-3">
        ...
        <div className="space-y-2 text-sm text-blue-800">
          ...
          <p className="flex items-start gap-2">
            ...
            <span>Envía el pago de <strong className="text-blue-900">${amount} USD</strong> a:</span>
          </p>
          ...
          <p className="flex items-start gap-2">
            ...
            <span>En el concepto/detalle del pago, escribe tu <strong>email de registro</strong></span>
          </p>
          <p className="flex items-start gap-2">
            <span className="font-bold min-w-[20px]">4.</span>
            <span>Haz clic en <strong>"Confirmar Pago"</strong> abajo para notificarnos</span>
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="bg-green-50 border-2 border-green-300 rounded-lg p-4 space-y-3">
        ...
        <div className="space-y-2 text-sm text-green-800">
          <p className="flex items-start gap-2">
            <span className="font-bold min-w-[20px]">1.</span>
            <span>Realiza la transferencia de <strong className="text-green-900">${amount} USD</strong> a la siguiente cuenta:</span>
          </p>
          <div className="bg-white border border-green-200 rounded p-3 space-y-1">
            <p><span className="text-green-600">Banco:</span> <strong>Banco General</strong></p>
            <p><span className="text-green-600">Cuenta:</span> <strong>04-72-99-123456-7</strong></p>
            <p><span className="text-green-600">Nombre:</span> <strong>Cuponea Inc.</strong></p>
            <p><span className="text-green-600">Tipo:</span> <strong>Cuenta de Ahorros</strong></p>
          </div>
          <p className="flex items-start gap-2">
            <span className="font-bold min-w-[20px]">2.</span>
            <span>En el concepto coloca tu <strong>email de registro</strong></span>
          </p>
          <p className="flex items-start gap-2">
            <span className="font-bold min-w-[20px]">3.</span>
            <span>Envía el comprobante a: <strong className="text-green-900">pagos@cuponea.com</strong></span>
          </p>
          <p className="flex items-start gap-2">
            <span className="font-bold min-w-[20px]">4.</span>
            <span>Haz clic en <strong>"Confirmar Pago"</strong> abajo para notificarnos</span>
...
export default function CuponeadorClientPayment() {
  ...
  const cuponeadorId = urlParams.get('cuponeador');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (cuponeadorData.length > 0) {
        ...
        const users = await base44.entities.User.filter({ id: cuponeadorData[0].user_id });
        if (users.length > 0) {
      ...
      if (cuponeadorUser?.email) {
        await base44.integrations.Core.SendEmail({
          ...
          body: `
            ...
            ${selectedPlan.recurring > 0 ? `- Comisión residual: $${selectedPlan.recurring}/mes x 6 meses` : ''}
          ...
          <CardContent className="p-8 space-y-6">
            ...
            <div>
              ...
              <p className="text-gray-600">
                Hemos recibido la notificación de pago para {prospect?.business_name}
              </p>
            </div>
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-2">
              ...
              <p className="text-blue-700 text-sm">
                Verificaremos el pago en las próximas 24 horas y activaremos la cuenta del cliente.
              </p>
            </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 px-4 py-8">
      <div className="max-w-2xl mx-auto">
        ...
        <Card className="mb-6 bg-white shadow-lg">
          ...
          <CardContent className="p-6 space-y-4">
            <div>
              ...
              <p className="text-lg font-bold text-gray-900">{prospect?.business_name}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">${selectedPlan.price}</p>
              </div>
            </div>

            <div className="bg-green-50 border border-green-200 rounded-lg p-3">
              <p className="text-sm text-green-800">
                <strong>Tu comisión:</strong> ${selectedPlan.commission} (por cierre)
                {selectedPlan.recurring > 0 && (
                  <span className="block mt-1">
            ...
            {cuponeadorUser && (
              <div className="pt-3 border-t">
                ...
                <p className="text-sm font-medium text-gray-700">{cuponeadorUser.full_name}</p>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4 mb-6">
              <Button
                ...
                className={`h-20 flex-col ${paymentMethod === 'yappy' ? 'bg-blue-600 hover:bg-blue-700' : ''}`}
              >
              ...
              <Button
                ...
                className={`h-20 flex-col ${paymentMethod === 'bank_transfer' ? 'bg-green-600 hover:bg-green-700' : ''}`}
              >
            ...
            <PaymentInstructions method={paymentMethod} amount={selectedPlan.price} />
          </CardContent>
        </Card>
        ...
        <Card className="mb-6 bg-yellow-50 border-yellow-300">
          <CardContent className="p-4 flex items-start gap-3">
            ...
            <div className="text-sm text-yellow-800">
              ...
              <p>
                El cliente debe realizar el pago según las instrucciones. Una vez completado, 
                haz clic en "Confirmar Pago" para notificar a nuestro equipo. Verificaremos 
                el pago en 24 horas y activaremos la cuenta.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <p className="text-xs text-gray-500 text-center mt-4">
          Al confirmar, declaras que el cliente ha realizado el pago según las instrucciones proporcionadas
        </p>
      </div>
    </div>
  );

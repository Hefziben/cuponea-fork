...
export default function Home() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Try to get current user, handle auth errors gracefully
      try {
        ...
        // If it's a 403, user is not authenticated - redirect to Welcome
        ...
        // For other errors, continue without user (public mode)
      ...
      if (currentUser?.account_type === 'business' || currentUser?.account_type === 'cuponeador') {
        setIsLoading(false);
        return;
      }
      ...
      // Load business data
      const businessIds = [...new Set(couponsData.map((c) => c.business_id).filter((id) => id))];
      let businessData = {};
      ...
      // If it's an auth error, redirect to Welcome
  ...
  // Show loading if there's an auth error and we're redirecting
  if (authError) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-teal-50 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#9b59b6] mx-auto mb-4"></div>
  ...
  if (user?.account_type === 'cuponeador') {
    window.location.href = createPageUrl("CuponeadorDashboard");
    return null;
  }
    ...
    <div className="bg-slate-50 mx-auto px-4 py-6 max-w-md space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 text-sm">Encuentra los mejores descuentos</p>
          </div>
          <Link to={createPageUrl("NearbyOffers")} className="flex-shrink-0">
        ...
        <div className="relative">
          ...
          <Button size="sm" variant="ghost" className="absolute right-2 top-1/2 transform -translate-y-1/2 hover:bg-gray-100">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="font-semibold text-gray-900 text-lg">Para Ti, con IA</h3>
            </div>
            <AiFeatureModal userType={user?.account_type}>
              <Button variant="ghost" size="sm" className="text-[var(--primary)] hover:text-[var(--primary-dark)] hover:bg-purple-50 text-xs">
                ¿Cómo funciona?
              </Button>
            </AiFeatureModal>
          </div>
          <div className="flex space-x-3 overflow-x-auto pb-2 -mx-4 px-4">
            {aiCoupons.length > 0 ? aiCoupons.map((coupon) =>
            ...
            <div className="space-y-2">
              ...
              {!user && (
                <p className="text-sm text-gray-500">
                  Los comerciantes pueden empezar a crear cupones desde la sección "Crear"
                </p>
              )}
            </div>
      ...
      <div className="space-y-3">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-semibold text-gray-900">Cupones Destacados</h3>
          </div>
      ...
      <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold text-gray-900">
              {selectedCategory === "all" ? "Todos los Cupones" :
            categories.find((c) => c.id === selectedCategory)?.name || "Cupones"}
            </h3>
          ...
          <Link to={createPageUrl("Categories")}>
                <Card className="bg-[var(--primary)] text-center p-4 rounded-lg border shadow-sm hover:shadow-md transition-all cursor-pointer border-purple-200">
                  <div className="space-y-2">
                    <p className="text-slate-50 font-semibold">Ver más cupones</p>

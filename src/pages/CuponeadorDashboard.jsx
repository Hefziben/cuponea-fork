...
export default function CuponeadorDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (cuponeadorInfo.length > 0) {
        ...
        const commissionData = await base44.entities.Commission.filter({ cuponeador_id: cuponeadorInfo[0].id });
        setCommissions(commissionData);

        // Load game commissions
        const gameCommissionData = await base44.entities.GameCommission.filter({ cuponeador_id: cuponeadorInfo[0].id });
  ...
  const getNextLevelRequirement = (currentClients) => {
    if (currentClients < 5) return { level: "Plata", needed: 5 - currentClients };
    if (currentClients < 15) return { level: "Oro", needed: 15 - currentClients };
    if (currentClients < 30) return { level: "Diamante", needed: 30 - currentClients };
  ...
  const totalGameEarnings = gameCommissions.reduce((sum, gc) => sum + gc.amount, 0);
  const pendingGameEarnings = gameCommissions.filter(gc => gc.status === 'pending').reduce((sum, gc) => sum + gc.amount, 0);
    ...
    <div className="px-4 py-6 max-w-md mx-auto space-y-6">
      ...
      <div className="text-center">
        <div className="flex items-center justify-center space-x-2 mb-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Dashboard Cuponeador</h1>
        </div>
        <p className="text-gray-600">¡Hola {user?.full_name}! Aquí está tu rendimiento</p>
      </div>
      ...
      <Card className="bg-gradient-to-r from-purple-500 to-indigo-600 text-white">
        <CardContent className="p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <Badge className={`${getLevelColor(cuponeadorData.level)} text-sm font-bold`}>
                Nivel {cuponeadorData.level.charAt(0).toUpperCase() + cuponeadorData.level.slice(1)}
              </Badge>
              <div className="text-2xl font-bold mt-1">#{cuponeadorData.rank_position || 'N/A'}</div>
          ...
          {nextLevel.needed > 0 && (
            <div className="bg-white/20 rounded-lg p-3">
              ...
              <Progress 
                value={(cuponeadorData.clients_registered / (cuponeadorData.clients_registered + nextLevel.needed)) * 100} 
                className="h-2"
              />
      ...
      <Card>
        ...
        <CardContent className="grid gap-3">
          <Link to={createPageUrl("CuponeadorProspecting")}>
            <Button className="w-full justify-start bg-blue-600 hover:bg-blue-700">
              <Briefcase className="w-4 h-4 mr-2" />
              Gestionar Clientes
            </Button>
          </Link>
          <Link to={createPageUrl("NearbyOffers")}>
            <Button className="w-full justify-start bg-green-600 hover:bg-green-700">
              <Target className="w-4 h-4 mr-2" />
              Prospectar Nuevos Comercios
            </Button>
          </Link>
          <Link to={createPageUrl("CuponeadorSalesCoach")}>
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              {commissions.slice(0, 2).map((commission) => (
                <div key={commission.id} className="flex justify-between items-center py-2 border-b last:border-b-0">
                  <div>
                    <div className="font-medium text-sm">
                      {commission.commission_type === 'cierre' ? 'Cierre' : 
                       commission.commission_type === 'volumen' ? 'Bono Volumen' : 'Residual'}
                    </div>
                    <div className="text-xs text-gray-500">{commission.period || 'Este mes'}</div>
                  </div>
                  <div className="text-right">
                    <div className="font-bold text-green-600">${commission.amount}</div>
                    <Badge variant={commission.status === 'pagado' ? 'default' : 'secondary'} className="text-xs">
                      {commission.status}
                    </Badge>
                  </div>
                </div>
              ...
              {gameCommissions.slice(0, 2).map((gameComm) => (
                <div key={gameComm.id} className="flex justify-between items-center py-2 border-b last:border-b-0 bg-purple-50">
                  <div>
                  ...
                  <div className="text-right">
                    <div className="font-bold text-purple-600">${gameComm.amount}</div>
      ...
      <Card className="bg-yellow-50 border-yellow-200">
        ...
        <CardContent>
          <div className="space-y-2 text-sm text-yellow-700">
            <div>• Visita comercios locales y explícales los beneficios de Cuponea</div>
            <div>• Enseña a tus clientes a activar "Juegos" para ganar comisiones pasivas</div>
            <div>• Usa redes sociales para compartir historias de éxito</div>
            <div>• Participa en eventos empresariales de tu ciudad</div>
            <div>• Mantén contacto con tus clientes para asegurar su retención</div>
          </div>
        </CardContent>
      </Card>
    </div>

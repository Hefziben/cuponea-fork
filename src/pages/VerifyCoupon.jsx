...
import { CheckCircle, AlertCircle, ArrowLeft, QrCode, Bell } from "lucide-react";
  ...
  const verifyCoupon = async () => {
    ...
    try {
      ...
      if (couponDetail.business_id) {
        const businessData = await Business.filter({ id: couponDetail.business_id });
      ...
      const now = new Date();
      const validUntil = couponDetail.valid_until ? new Date(couponDetail.valid_until) : null;
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-blue-50 px-4 py-8">
      <div className="max-w-md mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <h1 className="text-xl font-bold text-gray-900">Verificación de Cupón</h1>
          <div className="w-10" />
        </div>

        {verificationStatus === 'success' && coupon && (
          <div className="space-y-6">
            {/* Status Success */}
            <Card className="bg-green-50 border-green-200">
              <CardContent className="p-6 text-center">
                <CheckCircle className="w-12 h-12 text-green-500 mx-auto mb-4" />
                <h2 className="text-xl font-bold text-green-800 mb-2">¡Cupón Válido!</h2>
                <p className="text-green-700">Este cupón está listo para ser canjeado</p>
              </CardContent>
            </Card>
                ...
                {business && (
                  <div className="border-t pt-4">
                    ...
                    <div className="flex items-center space-x-3">
                      {business.logo_url && (
                        <img src={business.logo_url} alt={business.name} className="w-10 h-10 rounded-full" />
                      )}
                      <div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 text-sm">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-medium capitalize">{coupon.category}</p>
                    </div>
                    <div>
                      <span className="text-gray-600">Usos:</span>
                      <p className="font-medium">
                        {coupon.current_uses || 0}{coupon.max_uses ? `/${coupon.max_uses}` : ' usos'}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        {verificationStatus === 'redeemed' && (
          <Card className="bg-blue-50 border-blue-200">
            <CardContent className="p-8 text-center space-y-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-blue-800">¡Cupón Canjeado!</h2>
                <p className="text-blue-700 mt-2">El cupón se ha usado exitosamente</p>
              </div>
              ...
              <div className="bg-blue-100 rounded-lg p-4">
                <p className="text-sm text-blue-800">
                  Gracias por usar Cuponea. ¡Esperamos que hayas disfrutado tu descuento!
                </p>
              </div>
              <Button 
            ...
            <CardContent className="p-8 text-center space-y-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-red-800">
                  {verificationStatus === 'expired' ? 'Cupón Expirado' : 'Cupón No Válido'}
                </h2>

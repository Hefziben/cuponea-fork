  ...
  const loadSavedCoupons = async () => {
    ...
    try {
      ...
      if (savedData.length > 0) {
        ...
        const filteredCoupons = allCoupons.filter(c => couponIds.includes(c.id));
        setCoupons(filteredCoupons);

        // Get business data
        const businessIds = [...new Set(filteredCoupons.map(c => c.business_id).filter(id => id))];
        ...
        <Card className="p-8 text-center bg-purple-50 border-purple-100">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <h3 className="font-bold text-xl text-gray-900">Sin cupones guardados</h3>
              <p className="text-gray-600">
                Cuando encuentres cupones que te interesen, guárdalos aquí para usarlos más tarde.
              </p>
            </div>
            <Button 

...
export default function BusinessSetup() {
  ...
  const verifyCuponeadorCode = async () => {
    ...
    try {
      ...
      const cuponeadores = await Cuponeador.filter({ cuponeador_code: cuponeadorCode.trim() });
      ...
      if (cuponeadores.length > 0) {
        ...
        if (cuponeadorUser) {
          ...
          alert(`¡Código verificado! Serás atendido por ${cuponeadorUser.full_name}`);
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Error al guardar la información del negocio. Por favor, inténtalo de nuevo.");
    }
    setIsLoading(false);
  };
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="text-center">
            <h1 className="text-xl font-bold text-gray-900">Configura tu Negocio</h1>
          </div>
          <div className="w-10" />
        </div>
            ...
            {!cuponeadorVerified ? (
              <div className="space-y-3">
                <p className="text-sm text-green-700">
                  Si un cuponeador te refirió, ingresa su código para recibir atención personalizada y beneficios adicionales.
                </p>
                <div className="flex space-x-2">
                  <Input
                    placeholder="Código del cuponeador (ej: CUP123ABC)"
                    value={cuponeadorCode}
                    onChange={(e) => setCuponeadorCode(e.target.value.toUpperCase())}
              ...
              <div className="bg-white rounded-lg p-4 border-l-4 border-green-500">
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <span className="font-semibold text-green-800">Cuponeador Verificado</span>
                </div>
                <div className="text-sm text-gray-700">
                  <p><strong>Nombre:</strong> {cuponeadorInfo.name}</p>
                  <p><strong>Código:</strong> {cuponeadorInfo.code}</p>
                  <p><strong>Nivel:</strong> <Badge className="capitalize">{cuponeadorInfo.level}</Badge></p>
                  <p><strong>Clientes atendidos:</strong> {cuponeadorInfo.clients}</p>
                </div>
                <Button
                  variant="ghost"
                  size="sm"
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-4">
                ...
                <div>
                  <Label htmlFor="business_whatsapp">WhatsApp (opcional)</Label>
                  <Input
              ...
              <Button
                type="submit"
                disabled={isLoading || !formData.business_name || !formData.business_category || !formData.business_description || !formData.business_address}

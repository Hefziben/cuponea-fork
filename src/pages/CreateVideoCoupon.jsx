...
const templates = [
  { id: 'custom', name: 'Video Personalizado', description: 'Sube tu propio video', icon: Upload },
  { id: 'animated_basic', name: 'Animación Básica', description: 'Texto + Logo animado', icon: Wand2 },
  { id: 'countdown', name: 'Cuenta Regresiva', description: 'Video con timer', icon: Gamepad2 },
  { id: 'treasure_chest', name: 'Cofre del Tesoro', description: 'Juego de selección', icon: Gamepad2 },
  { id: 'scratch_card', name: 'Raspa y Gana', description: 'Tarjeta raspable', icon: Gamepad2 }
];

const gameTypes = [
  { id: 'none', name: 'Sin Juego', description: 'Solo video informativo' },
  { id: 'tap_to_claim', name: 'Toca para Reclamar', description: 'Usuario debe tocar en el momento justo' },
  { id: 'countdown', name: 'Cuenta Regresiva', description: 'Atrapar cupón antes de que termine' },
  { id: 'choose_box', name: 'Elige un Cofre', description: 'Seleccionar entre 3 opciones' },
  { id: 'scratch', name: 'Raspa y Gana', description: 'Raspar para revelar descuento' }
];
...
export default function CreateVideoCoupon() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const hasAdvancedPlan = ['avanzado', 'corporativo'].includes(currentUser.subscription_plan);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.coupon_id || (!formData.video_url && formData.template_used === 'custom')) {
      alert('Completa todos los campos requeridos');
      return;
    }

    // Check if user can create video-coupon
    const hasAdvancedPlan = ['avanzado', 'corporativo'].includes(user.subscription_plan);
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-2xl font-bold text-gray-900">Crear Video-Cupón</h1>
            </div>
            <p className="text-sm text-gray-600">Potencia tus ofertas con video</p>
          </div>
        </div>

        {/* Credits/Plan Status Banner */}
        {!hasAdvancedPlan ? (
          <Card className={`${creditsRemaining > 0 ? 'bg-gradient-to-r from-blue-500 to-cyan-500' : 'bg-gradient-to-r from-gray-500 to-gray-600'} text-white`}>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-white/90">
                    {creditsRemaining > 0 ? (
                      <>Te quedan <strong>{creditsRemaining}</strong> video-cupón{creditsRemaining !== 1 ? 'es' : ''} gratis</>
                    ) : (
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-white/90">Video-cupones ilimitados</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <Select value={formData.coupon_id} onValueChange={(value) => setFormData({...formData, coupon_id: value})}>
                <SelectTrigger>
              ...
              {templates.map((template) => {
                ...
                return (
                  <Button
                    key={template.id}
                    type="button"
                    variant={formData.template_used === template.id ? 'default' : 'outline'}
                    className="w-full justify-start h-auto p-4"
                    onClick={() => setFormData({...formData, template_used: template.id})}
                  >
          ...
          <Card>
            ...
            <CardContent>
              <Select value={formData.urgency_type} onValueChange={(value) => setFormData({...formData, urgency_type: value})}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <Button
            ...
            disabled={isUploading || (!formData.video_url && formData.template_used === 'custom') || (!hasAdvancedPlan && creditsRemaining <= 0)}
          >
            {isUploading ? (
              <>
          ...
          {!hasAdvancedPlan && creditsRemaining > 0 && (
            <p className="text-xs text-center text-gray-600">
              Crearás tu video-cupón #{user && user.video_coupon_credits ? (user.video_coupon_credits - creditsRemaining + 1) : ''} de {user && user.video_coupon_credits ? user.video_coupon_credits : ''} gratis
            </p>
          )}
        </form>
      </div>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-base">
              ¡Felicidades! Has probado los video-cupones gratuitos. ¿Te gustó el resultado?
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-4 border-2 border-purple-200">
              ...
              <ul className="space-y-2 text-sm text-gray-700">
                <li className="flex items-center gap-2">
                  ...
                  <span><strong>Video-cupones ilimitados</strong></span>
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-green-600" />
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
              <p className="text-sm text-yellow-800">
                <strong>💡 Tip:</strong> Los comercios con video-cupones obtienen 3x más canjes y engagement que con cupones tradicionales.
              </p>
            </div>
          </div>

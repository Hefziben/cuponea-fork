...
import { Map, Pin, Compass, Filter, MapPin, Bell, Sparkles, Plus, List, Search, Gamepad2, Camera, Phone, Building } from "lucide-react";
...
export default function NearbyOffers() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (currentUser && currentUser.account_type === 'cuponeador') {
        const prospectData = await base44.entities.Prospect.filter({ cuponeador_id: currentUser.id });
        setProspects(prospectData);
      }

      const offers = await base44.entities.Coupon.filter({ is_active: true }, '-created_date', 10);
      ...
      const offersWithLocation = offers.map((offer, index) => ({
        ...offer,
        distance: aiResponse.locations[index]?.distance || `${Math.floor(Math.random() * 1000 + 100)}m`,
        location: aiResponse.locations[index]?.location || "Centro de la ciudad",
  ...
  const handleSubmitProspect = async (e) => {
    e.preventDefault();
    if (!prospectData.business_name || !prospectData.address || !prospectData.phone) {
      alert("Por favor completa el nombre del negocio, dirección y teléfono.");
      return;
    }
    ...
    <div className="bg-slate-50 mx-auto px-4 py-6 max-w-md space-y-6">
      ...
      <div className="text-center space-y-2">
        <div className="flex items-center justify-center space-x-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900">
            {isCuponeador && isProspectingMode ? 'Modo Prospección' : 'Ofertas Cerca de Ti'}
          </h1>
        </div>
        <p className="text-gray-600">
          {isCuponeador && isProspectingMode ? 'Gestiona y añade nuevos comercios' : 'Descubre promociones a tu alrededor.'}
        </p>
      </div>
        ...
        <Card className="bg-gradient-to-r from-purple-50 to-indigo-50 border-purple-200">
          <CardContent className="p-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <h3 className="font-semibold text-purple-800 mb-2">Funcionalidad de Juegos para Cuponeadores</h3>
                <div className="text-sm text-purple-700 space-y-1">
                  <p>• Ayuda a tus comercios clientes a aumentar la viralidad de sus cupones</p>
                  <p>• Gana comisiones adicionales cuando los cupones compartidos son reclamados</p>
                  <p>• Enseña a los comercios cómo activar el modo juego en sus cupones</p>
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-4 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-green-600">{prospects.length} comercios en seguimiento</p>
              </div>
            </div>
            <Button 
              onClick={handleToggleProspecting} 
              variant={isProspectingMode ? 'default' : 'outline'} 
              className={isProspectingMode ? 'bg-green-600 hover:bg-green-700 text-white' : ''}
            >
      ...
      <Card>
        <CardContent className="p-2">
          <div 
            ...
            style={{
              backgroundImage: 'url(https://images.unsplash.com/photo-1584931214441-835623425f38?q=80&w=1740&auto=format&fit=crop)',
              backgroundSize: 'cover',
              backgroundPosition: 'center'
            }}
            onClick={isCuponeador && isProspectingMode ? handleAddProspect : undefined}
          >
            <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
              <div className="text-center bg-white/95 backdrop-blur-sm p-6 rounded-lg shadow-lg">
                ...
                <h3 className="font-bold text-lg text-gray-900">Mapa Interactivo</h3>
      ...
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-gray-800">
          {isCuponeador && isProspectingMode ? 'Prospectos Cercanos' : 'Promociones Cercanas'}
        </h3>
        {isCuponeador && isProspectingMode ? (
          <Button size="sm" className="space-x-2 bg-green-600 hover:bg-green-700 text-white" onClick={handleAddProspect}>
        ...
        {isLoading ? (
          ...
          prospects.length > 0 ? (
            prospects.map((prospect) => (
              <Card key={prospect.id} className="border-2 border-blue-200 bg-blue-50">
                <CardContent className="p-4">
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        <h4 className="font-semibold text-gray-800">{prospect.business_name}</h4>
                        <Badge variant="secondary" className="capitalize">{prospect.status}</Badge>
                      </div>
                      <div className="text-sm text-gray-600 space-y-1">
                        ...
                        {prospect.notes && (
                          <p className="text-xs text-gray-500 italic mt-1">{prospect.notes}</p>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <p className="text-center text-gray-500 py-8">No hay prospectos en seguimiento. ¡Añade uno tocando el mapa!</p>
          )
        ) : (
          nearbyOffers.length > 0 ? (
            nearbyOffers.map((offer) => (
              <Card key={offer.id} className={`border-2 transition-all ${offer.isNearby ? 'border-orange-200 bg-orange-50' : 'border-gray-100'}`}>
                <CardContent className="p-4 flex items-center space-x-4">
                  <div className={`p-3 rounded-lg ${offer.isNearby ? 'bg-orange-100' : 'bg-purple-100'}`}>
                    <Pin className={`w-6 h-6 ${offer.isNearby ? 'text-[var(--urgent)]' : 'text-[var(--primary)]'}`} />
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <h4 className="font-semibold text-gray-800">{getDiscountText(offer)}</h4>
                      {offer.isNearby && (
            ...
            <p className="text-center text-gray-500 py-8">No hay ofertas cercanas disponibles.</p>
          )
        )}
      </div>
        ...
        <Dialog open={showLocationAlert} onOpenChange={setShowLocationAlert}>
          <DialogContent className="max-w-sm">
              ...
              <DialogDescription className="pt-4">
                {alertOffer && (
                  <div className="text-center space-y-3">
                    ...
                    <div>
                      <h3 className="font-bold text-lg text-gray-900">{alertOffer.businessName}</h3>
                      <p className="text-gray-700 mt-1">{alertOffer.offer}</p>
                    </div>
            ...
            <DialogFooter className="sm:justify-center">
              <Button variant="outline" onClick={() => setShowLocationAlert(false)}>
                Más tarde
              </Button>
              <Button 
      ...
      <Dialog open={showProspectForm} onOpenChange={setShowProspectForm}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Registra todos los detalles del comercio para un mejor seguimiento
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmitProspect} className="space-y-4 py-4">
              ...
              <div className="space-y-3 mt-2">
                {prospectPhotoUrl ? (
                  <div className="relative">
                    <img src={prospectPhotoUrl} alt="Prospect" className="w-full h-40 object-cover rounded-lg" />
                    <Button 
                      type="button"
                      variant="outline" 
                      size="sm"
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                    ...
                    <p className="text-sm text-gray-500 mb-3">Toma o sube una foto del comercio</p>
                    <Button 
                      type="button"
                      variant="outline" 
                ...
                <input
                  ...
                  className="hidden"
                />
                <p className="text-xs text-gray-500">* La foto ayuda a identificar el local más tarde</p>
              </div>
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setProspectData({...prospectData, business_name: e.target.value})}
            ...
            <div>
              ...
              <Input
                ...
                required
              />
              <p className="text-xs text-gray-500 mt-1">Incluye referencias para encontrar el local fácilmente</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setProspectData({...prospectData, business_type: e.target.value})}
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setShowProspectForm(false)}>
                Cancelar
              </Button>

...
export default function Layout({ children, currentPageName }) {
  ...
  useEffect(() => {
    const loadUser = async () => {
      try {
        ...
        !location.pathname.includes('SelectPlan') &&
        !location.pathname.includes('Payment') &&
        !location.pathname.includes('CuponeadorSignup')) {
  ...
  const specialPages = ['Welcome', 'BusinessSetup', 'SelectPlan', 'Payment', 'ClaimCoupon', 'VerifyCoupon', 'CuponeadorSignup'];
  ...
  navigationItems = navigationItems.filter((item) => {
    if (isBusiness) return item.forBusiness === true || !item.forUser && !item.forCuponeador;
    if (isCuponeador) return item.forCuponeador === true || !item.forUser && !item.forBusiness;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-teal-50">
          ...
          <div className="flex items-center space-x-3">
            <Link to={createPageUrl("Home")} className="flex items-center">
              <img
                ...
                alt="Cuponea Logo" className="h-8 w-auto" />
      ...
      <main className="bg-transparent pb-20 min-h-screen">
        {children}
      </main>

      {/* AI Chat Assistant FAB */}
      {user && (user.account_type === 'user' || user.account_type === 'cuponeador') &&
      <AiChatAssistant user={user}>
          <Button
          className="fixed bottom-24 right-4 h-14 w-14 rounded-full bg-[var(--primary)] text-white shadow-lg hover:scale-110 transition-transform z-40">

...
export default function ClaimCoupon() {
  ...
  useEffect(() => {
    setShareToken(window.location.pathname.split("/").pop());
  }, []);
  ...
  useEffect(
    () => {
      if (timeLeft === null || !sharedCoupon) return;
    ...
    [sharedCoupon, timeLeft]
  );

  const loadSharedCoupon = async () => {
    try {
      const sharedData = await SharedCoupon.filter({
        ...
        <div className="text-center space-y-6 animate-in fade-in duration-500">
          ...
          <div className="space-y-4">
            ...
            <p className="text-lg text-gray-600">
              Has reclamado exitosamente tu cupón de{" "}
              {sharedCoupon.recipient_name}
            </p>
          ...
          <Button
            onClick={() => router.push(createPageUrl("Saved"))}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <p className="text-red-800">
                💡 <strong>Tip:</strong> No pierdas las oportunidades, mantente
                pendiente de los nuevos cupones en CUPONEA
              </p>
            </div>
          </div>
          <Button
    ...
    return (
      <div className="space-y-6">
        ...
        >
          <CardContent className="p-6 text-center">
            <div className="flex items-center justify-center space-x-2 mb-4">
              <Clock
                className={`w-5 h-5 ${showBomb
                  ? "text-red-500"
                  : "text-orange-500"}`}
              />
              <span
                className={`font-semibold ${showBomb
                  ? "text-red-700"
                  : "text-orange-700"}`}
              >
                Tiempo restante
            ...
            {timeLeft && (
              <div className="flex justify-center space-x-4">
                <div className="text-center">
                  <div
                    className={`text-3xl font-bold ${showBomb
                      ? "text-red-600"
                      : "text-orange-600"}`}
                  >
                ...
                <div className="text-center">
                  <div
                    className={`text-3xl font-bold ${showBomb
                      ? "text-red-600"
                      : "text-orange-600"} ${timeLeft.hours === 0 &&
                    timeLeft.minutes < 5
                      ? "animate-pulse"
                      : ""}`}
                  >
        ...
        <Button
          ...
          className={`w-full py-6 text-lg font-bold ${showBomb

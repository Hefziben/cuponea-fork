...
export default function VideoCouponFeed() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Load video coupons
      const videoData = await base44.entities.VideoCoupon.filter(
        { is_active: true },
        "-views_count",
        50
      );
        ...
        try {
          ...
          if (preferences.length > 0) {
            ...
            const sortedVideos = [...videoData].sort((a, b) => {
              ...
              const scoreA = userCategories.includes(couponA?.category)
                ? 10
                : 0;
              const scoreB = userCategories.includes(couponB?.category)
                ? 10
                : 0;
  ...
  const handleClaim = async () => {
    if (selectedVideo && user) {
      try {
        ...
        router.push(createPageUrl("Saved"));
      } catch (error) {
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-6">
        ...
        {isLoading ? (
          <div className="grid grid-cols-2 gap-4">
            {Array(6)
              .fill(0)
              .map((_, i) => <Skeleton key={i} className="h-64 rounded-lg" />)}
          </div>
        ) : filteredVideos.length > 0 ? (
          <div className="grid grid-cols-2 gap-4">
                ...
                >
                  <div className="relative">
                    ...
                    <div
                      ...
                      style={{
                        backgroundImage: videoCoupon.thumbnail_url
                          ? `url(${videoCoupon.thumbnail_url})`
                          : undefined,
